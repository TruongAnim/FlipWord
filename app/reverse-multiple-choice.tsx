...
export default function ReverseMultipleChoiceScreen() {
    ...
    const handleSelectOption = (option: string) => {
        ...
        trackingRepository.logAttempt(currentQuestion.targetWord.id, correct, 'reverse_multiple_choice');
        if (correct) {
    ...
    const handleNext = () => {
        if (currentIndex < questions.length - 1) {
            ...
            trackingRepository.logSession(duration, 'reverse_multiple_choice');
            setIsCompleted(true);
        }
    };
        ...
        <SafeAreaView className="flex-1 bg-orange-50">
            ...
            <View className="flex-row items-center px-4 py-2 bg-orange-50 z-10">
                ...
                <View className="flex-1 items-end">
                    <Text className="text-orange-600 font-medium text-lg">{currentIndex + 1}/{questions.length}</Text>
                </View>
            </View>

            {/* Timer Bar */}
            <GameTimer
                duration={GameConfig.REVERSE_CHOICE_DURATION}
                onTimeout={() => {
                    const duration = Math.floor((Date.now() - startTimeRef.current) / 1000);
                    trackingRepository.logSession(duration, 'reverse_multiple_choice');
                    setIsCompleted(true);
